...
grep -Eq '(vmx|svm)' /proc/cpuinfo || die "Enable Intel VT-x in firmware first"
...
for service in dbus virtlockd virtlogd libvirtd; do
  [[ -d "/etc/sv/$service" ]] || die "Missing service: $service"
...
sudo virsh -c qemu:///system list >/dev/null || die "libvirt did not become ready"
...
if ! sudo virsh -c qemu:///system net-start default >/dev/null 2>&1; then
  sudo virsh -c qemu:///system net-info default | grep -Eq 'Active:[[:space:]]+yes' || die "Default NAT network failed"
fi
